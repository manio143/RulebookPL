...
* Strata tłuczka powoduje, że nie może on zostać użyty do zbicia zawodnika, dopóki nie wypełni się procedury przekazania piłki.
...
* Jeśli zawodnik dostaje drugą żółtą kartkę, to po pokazaniu żółtej kartki zawodnik zostaje usunięty z boiska. Zamiennik odbywa za niego czas kary.
* Gra jest wznowiona.
...
* Jeśli zawodnik odmówi opuszczenia strefy graczy:
    ...
    * Jeśli zawodnik się opiera lub zaczyna być niebezpieczny, sędzia główny może zakończyć grę ze skutkiem poddania się drużyny zawodnika.
...
* Jeśli obie drużyny popełniły faul skutkujący poddaniem gry w tym samym zagraniu lub sytuacji, następuje ogłoszenie podwójnego poddania.
...
* Jeśli wiele niezwiązanych naruszeń zostanie popełnionych przez rezerwowych lub personel, powinny być one traktowane jako oddzielne sytuacje na rzecz tej zasady.
    * Jeśli powodowałoby to kilka kar dla _Speaking captain_ to odbywa on tylko jedną najcięższą (lub jego zamiennik w przypadku czerwonej), a pozostałe czasy kar są odbywane przez zawodników ściągniętych z boiska.
        ...
        * Jeśli _speaking captain_ dostaje w ten sposób dwie żółte kartki, to zostaje usunięty z boiska.
...
* Zdobycie bramki nie może wypuścić szukającego ze strefy kar zanim nie rozpocznie się jego czas kary.

### Faule po zakończeniu gry
...
* Kartki skutkują jedną lub dwiema minutami kary dla ukaranej osoby.
    ...
    * Jeśli zawodnik odsiaduje czas za kilka kar, to odsiaduje czas kolejnych kar w kolejności w jakiej był nimi ukarany.
    * Dwu-minutowa kara kończy się po tym jak zawodnik spędził pełne dwie minuty czasu gry w strefie kar.
    * Jedno-minutowa kara kończy się jeśli zawodnik spędził minutę czasu gry w strefie kar, lub przeciwna drużyna zdobyła punkty w dowolny sposób.
        * Pojedyńcze zdobycie punktów kończy jedną karę zawodnika odsiadującego najmniejszą pozostałą ilość jedno-minutowych kar.
        * Jeśli dwóch zawodników ma tyle samo pozostałych jedno-minutowych kar, to kończona jest kara z najmniejszym pozostałym czasem.
        * Jeśli dwóch zawodników ma tyle samo pozostałych jedno-minutowych kar i tyle samo czasu im pozostało to sędzia decyduje, któremu zawodnikowi kończy się jedna kara.
    * Jeśli faulowana drużyna zdobędzie punkty przed zatrzymaniem gry i nałożeniem jedno-minutowej kary, oraz żadna trwająca kara nie byłaby zakończona tymi punktami, to nałożona kara od razu się kończy.
        * Jeśli nakładane jest więcej niż jedna jedno-minutowa kara, to tylko jedna z nich jest od razu skończona.
        * Jeśli zawodnik karany był tylko jedną jedno-minutową karą i została ona skończona, to nie przenosi się on do strefy kar.
    * Jeśli zawodnik opuści strefę kar z dowolnego powodu, czas spędzony poza strefą kar nie jest liczony na rzecz tej kary.
...
* Czerwona kartka skutkuje dwu-minutową karą w strefie kar dla zamiennika.
* Niebieskie i żółte kartki skutkują jedno-minutową karą dla faulującego zawodnika.
* Jeśli osoba, odsiadująca karę za popełniony przez nią faul, otrzyma niebieską lub żółtą kartkę, to muszą spędzić dodatkową minutę w strefie kar.
* Jeśli osoba, odsiadująca karę jako zamiennik, otrzyma kartkę:
    ...
    * Faulujący zawodnik traktowany jest zgodnie z zasadami kar dla zamienników (patrz \myref{9.2.3}).
...
* Czas kary rozpoczyna się w momencie zagwizdania przez sędziego głównego na wznowienie gry.
...
* Jeśli zawodnik rezerwowy otrzymujący kartkę nie jest usunięty z boiska, to dochodzi do zamiany między nim i wyznaczonym zawodnikiem.
...
* Jeśli zawodnik rezerwowy zostaje usunięty z boiska, to wyznaczony zawodnik przechodzi do strefy kar odbyć karę.
...
* Kiedy przywilej się skończy, sędzia musi wstrzymać grę i rozdać odpowiednie kary.
...
* Jeśli gra zostaje wstrzymana w związku z tą procedurą, ale nikt nie jest ukarany, to nie było to opóźnioną karą.
...
### Alternatywne pozycje do wznowienia
